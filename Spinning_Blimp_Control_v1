...
void controlOutputs(float ifx, float ify, float ifz, float itx, float ity, float itz) {
  ...
  float term1 = kpz*radians(ifx)*cos(radians(itz));
  float term2 = kpz*radians(ify)*cos(radians(itz));
  ...
  float coscos   = cos(joytheta)*cos(radians(itz));
  float sincos   = sin(joytheta)*cos(radians(itz));

  // Cyclic pitch input
  ifx = joymag*(coscos  - sincos);
  ify = joymag*(-sincos - coscos);
